...
این سند برای افرادی نوشته شده که مخزن (Repository) را مدیریت می‌کنند تا مراحل انجام کارهای نگهداری دوره‌ای را فراموش نکنند.

## به‌روزرسانی نسخه `rustc`

برای ارتقای نسخه کامپایلر Rust:

- پوشه `target` را حذف کنید؛ در هر صورت قرار است همه چیز دوباره کامپایل شود.
- شماره نسخه را در فایل `.github/workflows/main.yml` تغییر دهید.
- شماره نسخه را در فایل `rust-toolchain` نیز به‌روزرسانی کنید. این کار باعث می‌شود نسخه مورد استفاده محلی شما از طریق `rustup` تغییر کند.
- شماره نسخه را در فایل `src/title-page.md` نیز اصلاح کنید.
- اسکریپت `./tools/update-rustc.sh` را اجرا کنید (برای جزئیات، کامنت‌های درون اسکریپت را ببینید)
- تغییرات ایجادشده را بررسی کنید:
	- فایل‌های تغییرکرده را با Git مشاهده کنید.
	- خروجی‌های موجود در `tmp/book-before` و `tmp/book-after` را مقایسه کنید.
	- اگر همه چیز درست بود، تغییرات را Commit کنید.
- عبارت `manual-regeneration` را جستجو کرده و دستورالعمل‌های آن بخش‌ها را برای به‌روزرسانی خروجی‌هایی که اسکریپت نمی‌تواند تولید کند دنبال کنید

## به‌روزرسانی مقدار `edition` در تمام مثال‌ها

برای به‌روزرسانی فیلد:

```
edition = "[YEAR]"
```

در تمام فایل‌های `Cargo.toml` مربوط به مثال‌های کتاب، اسکریپت زیر را اجرا کنید:
...
## به‌روزرسانی `edition` در تنظیمات mdBook

فایل‌های `book.toml` و `nostarch/book.toml` را باز کرده و مقدار `edition` در بخش `[rust]` را به نسخه جدید تغییر دهید.

## انتشار نسخه جدید مثال‌های کتاب (Listings)

برای تمام پروژه‌های مثال کتاب، فایل‌های فشرده (`.tar.gz`) منتشر می‌شوند و از طریق [GitHub Releases](https://github.com/rust-lang/book/releases) در دسترس قرار می‌گیرند.

اگر به دلیل تغییرات کد، ارتقای Rust یا تغییرات `rustfmt` نیاز به انتشار نسخه جدید داشتید:

- یک Git Tag جدید ایجاد کرده و آن را به GitHub Push کنید یا از طریق رابط کاربری GitHub [یک Release جدید بسازید](https://github.com/rust-lang/book/releases/new) و Tag جدیدی تعریف کنید.
- دستور `cargo run --bin release_listings` را اجرا کنید، این دستور فایل `tmp/listings.tar.gz` را تولید می‌کند.
- فایل `tmp/listings.tar.gz` را به Release Draft در GitHub اضافه کنید.
- Release را منتشر (Publish) کنید.

## افزودن یک مثال (Listing) جدید
